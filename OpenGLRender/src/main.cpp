...
std::shared_ptr<OpenGL::ViewerManager> viewer = nullptr;
...
int main() {
  ...
  //    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    ...
    if (!initSuccess) {
        ...
        glfwGetFramebufferSize(window, &frameWidth, &frameHeight);//获取实际的帧缓冲大小，存储在framewidth和framHeight中。
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // draw frame
            int outTex = viewer->drawFrame();//渲染1帧，返回渲染后的结果纹理。

            glDisable(GL_BLEND);
            glDisable(GL_DEPTH_TEST);
            glDepthMask(true);//启用深度缓冲写入
            glDisable(GL_CULL_FACE);
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);//设置多边形填充模式为填充（而不是线框或点）。

            glBindFramebuffer(GL_FRAMEBUFFER, 0);//绑定默认帧缓冲（即屏幕）。
